...
<?php
	...
	for($i=0;$i<count($listeofname);$i++){
		...
		if($date0212!=$nonfait){
			...
			$pas = nbparticipants($nom) + 0.5;//pour les groupes de plusieurs personnes
			if($j==0) {//première colonne
				$line1 .= "<tr></table><table>";
			...
			$bgcolor = "#c0c0c0";
			if($lien_copies) {
				$photo = photobord($nom,"#0a0");
				if(!$note) $bgcolor = "#fff";
				if($note=="0") $bgcolor = "#c0c0c0";
			}
			if(!$note) $photo = photobord($nom,"#fff");
			else $photo = photobord($nom,"#F2F5A9");
	...
	if($j!=0){
		while($j<$nbphotoslignes+1){
			$line1 .= "<td width=\"125px\">.</td>";
			$line2 .= "<td>.</td>";
			$j = $j + $pas;
		}
		$line1 .= "</tr>";	
...
<?php
	...
	$image .= "<img src=\"./graphe.php?notes=$texte_notes&filename=$filesave&noms=$texte_noms\"/></a>";
