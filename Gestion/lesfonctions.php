...
*/
	...
	function note2copie($nom2,$classe,$epreuve,$matiere){
		$resultat = "----";
		$fichieralire = "./files/$classe/$matiere/$epreuve.txt";
		if(file_exists($fichieralire)){
			$resultat = "Copie non trouvée";
	...
	function tabDplannings($laclasse,$lesdates,$ladate){
		...
		for($i=0;$i<count($lesdates);$i++){
			...
			$ligne2 .= "<td $theme><a href=\"planning.php?action=3&ladate=$part[1]/$part[0]\"><font color=\"yellow\">$part[1]<font></a></td>";//les jours
		}
	...
	$deroulant3 = "<SELECT name=\"elv\" id=\"elv\">";
