<?php include("./haut.php");
	...
	for($k=0;$k<$nbdepreuves;$k++) {
		...
		for($j=0;$j<count($leleve);$j++) {
			$somme_note[$j]=$somme_note[$j]+$coef[$j]*$note[$j];
			if($note[$j]!="") {
				$somme_coef[$j]=$somme_coef[$j]+$coef[$j];
				$classe_sum7[$k] += $note[$j]; //Somme des notes dans l'épreuve
	...
	//$image = barregraphe($lesmoyennes,$moyenne_delaclasse,"./files/$classe/_$matieres.txt",$lesnoms14);
